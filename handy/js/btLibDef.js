...
(function() {
	...
	Def.BT_ERR_ARG_4							=	-14;			// Error argument (the 4th argument)
	Def.BT_ERR_ARG_5							=	-15;			// Error argument (the 5th argument)
	Def.BT_ERR_ARG_6							=	-16;			// Error argument (the 6th argument)
	...
	Def.BT_ERR_FILE_OPEN						=	-401;			// Failed to open file
	Def.BT_ERR_FILE_READ						=	-402;			// Failed to read file
	...
	Def.BT_SYS_PRM_WAKEUP_ROUTE					=	5;				// [WakeUp] Communication route
	Def.BT_SYS_PRM_WAKEUP_PORT					=	7;				// [WakeUp] Communication port
	Def.BT_SYS_PRM_KIOSK_ENABLE					=	8;				// KIOSK mode
	Def.BT_SYS_PRM_KIOSK_DESKTOPIMAGE			=	9;				// KIOSK mode desktop image
	Def.BT_SYS_PRM_DESIGNTOOL_TYPE				=	10;				// DesignTool support type
	...
	// Communication route(BT_SYS_PRM_WAKEUP_ROUTE)
	Def.BT_SYS_WAKEUP_OFF						=	0;				// Wake up OFF
	Def.BT_SYS_WAKEUP_USB_COM					=	1;				// USB cradle (COM)
	Def.BT_SYS_WAKEUP_LANCRADLE					=	2;				// LAN cradle (CDC)
	Def.BT_SYS_WAKEUP_WLAN						=	3;				// Wireless LAN
	...
	Def.BT_SCAN_EXT_LIGHT_3D					=	0x0008;			// 3D lighting
	Def.BT_SCAN_EXT_LIGHT_ALL					=	0x000F;

	// 3D light setting(2D)
	Def.BT_SCAN_3D_MODE_RECESSED				=	0x0001;			// 3D(Recessed)
	Def.BT_SCAN_3D_MODE_RAISED					=	0x0002;			// 3D(Raised)
	Def.BT_SCAN_3D_MODE_ALL						=	0x0003;			// 3D(Recessed + Raised)
	...
	// Low Angle Direction(2D)
	Def.BT_SCAN_EXT_LOW_DIRECTION_TOP			= 0x01;				// Top
	Def.BT_SCAN_EXT_LOW_DIRECTION_RIGHT			= 0x02;				// Right
	Def.BT_SCAN_EXT_LOW_DIRECTION_LEFT			= 0x04;				// Left
	Def.BT_SCAN_EXT_LOW_DIRECTION_LOW			= 0x08;				// Low

	// Scan property ID
	// 1D/2D code
	Def.BT_SCAN_PROP_JAN						=	0x00001000;		// JAN property
	Def.BT_SCAN_PROP_JAN_EX						=	0x0000101D;		// Extended JAN property
	...
	Def.BT_SCAN_PROP_TOF_EX						=	0x00001710;		// TOF extended property
	...
	Def.BT_SCAN_PROP_3D_ILLUMINATION			=	0x000055B0;		// [2D]3D lighting
	Def.BT_SCAN_PROP_LIVEVIEW_DECODING			=	0x00004AA0;		// [2D]Display liveview decoding
	Def.BT_SCAN_PROP_ATTACHIMENT				= 	0x00004AB0;		// [2D]Light extended attachment
	...
	Def.BT_SCAN_PROP_EXT_LOW_DIRECTION			=	0x00004AE0;		// [2D]Low Angle Direction
	...
	Def.BT_SCAN_PROP_OCR_MIXED_POL_MODE			=	0x000055D0;		// Mixed Polarity
	...
	Def.BT_SCAN_EXT_LIGHT_BRIGHTNESS = function( _nolight, _normal, _multi, _low, _3D, _area ){
		...
		this.Brightness3D		= _3D;			// 3D lighting
	...
	Def.BT_SCAN_OCR_FORMAT_EACH = function(_CharMode,_digits,_alphabets,_marks,_L_alphabets){
		this.CharMode = _CharMode;				// Character type (BT_SCAN_OCR_DFMT_CHAR_MODE_XXX)
		this.digits = _digits;					// Number (BT_SCAN_OCR_DFMT_EACH_0 ~)
		this.alphabets = _alphabets;			// Alphabet (BT_SCAN_OCR_DFMT_EACH_A ~)
		this.marks = _marks;					// Mark (BT_SCAN_OCR_DFMT_EACH_HYPHEN ~)
		this.L_alphabets = _L_alphabets;		// Lower case alphabet (BT_SCAN_OCR_DFMT_EACH_A ~)
	};
